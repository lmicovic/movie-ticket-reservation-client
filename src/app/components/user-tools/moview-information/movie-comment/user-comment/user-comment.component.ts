...
export class UserCommentComponent implements OnInit{
  ...
  ngOnInit(): void {    
    ...
    this.userService.getUserImage(this.userComment.user.id).subscribe((response: Blob) => {

      
      // If User has not Profile Image - then set Default Image
      if(response.size === 0) {
        this.userComment.user.image = "../../../assets/images/user-profil-image-empty.png";
      }
      // If User have profile image - then decode Blob Profile Image in to URL Object, to be able to display in Browser.
